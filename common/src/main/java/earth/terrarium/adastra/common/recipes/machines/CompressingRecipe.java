...
public record CompressingRecipe(
        ...
        Ingredient ingredient, ItemStack result) implements CodecRecipe<ContainerRecipeInput> {
    ...
    public boolean matches(ContainerRecipeInput input, Level level) {
        ...
        if (!ingredient.test(container.getItem(1)))
            return false;
        if (!(container instanceof CompressorBlockEntity entity))
            return true;
