...
public record SpaceStationRecipe(List<IngredientHolder> ingredients,
        ...
        ResourceLocation structure) implements CodecRecipe<ContainerRecipeInput> {
    ...
    public static final MapCodec<SpaceStationRecipe> CODEC = RecordCodecBuilder.mapCodec(
            instance -> instance.group(
                    ...
                    ResourceKey.codec(Registries.DIMENSION).fieldOf("dimension")
                            .forGetter(SpaceStationRecipe::dimension),
    ...
    public static Optional<RecipeHolder<SpaceStationRecipe>> getSpaceStation(Level level,
            ResourceKey<Level> dimension) {
    ...
    public static boolean hasIngredients(Player player, Level level, SpaceStationRecipe recipe) {
        if (player.isCreative() || player.isSpectator())
            return true;
        return recipe.matches(new ContainerRecipeInput(player.getInventory()), level);
    }
    ...
    public static void consumeIngredients(Player player, Level level) {
        ...
        if (recipe == null)
            return;
        if (!recipe.value().matches(new ContainerRecipeInput(player.getInventory()), level))
            return;
